...
class DatasetInfo:
    ...
    eval_split: str
    extractor: Callable
    name: str = None # name of subset (task)
    data_dir: str = None
...
def get_questions_with_exemplars(
    ...
    '''
     return 一个few-shot样本列表，每个元素是一个QuestionWithExemplars实例
    '''
    ...
    ) # return row{"question", "choice", answer_idx}
...
def load_mmlu(n_shots, do_strong_shuffle):
    '''
     ...
     其中，50个task的train部分合成了一个大的auxiliary_train
     50个task中的每个task都被分为3个split：dev, test, validation
    '''
    all_qwes = list()
    for name in MMLU_NAMES:
        name_qwes = get_questions_with_exemplars(
            info=DatasetInfo(
                path="cais/mmlu",  # TODO 修改mmlu path
                name=name,
                exemplar_split="dev",
                eval_split="test",
                extractor=lambda row: {
